...
其中 `server` 会从 `server/config` 读取配置，并在启动时执行 Gorm 自动迁移。
...
这个脚本默认会把 Go 模块缓存和编译缓存放到 `server/.cache/go-mod` 与 `server/.cache/go-build`，避免占用 Docker 构建缓存空间。

这个脚本会：

- 自动读取 `deploy/docker-compose/.env`，若不存在则回退到根目录 `.env`
- 设置 `GOADMIN_ENV=dev`
- 设置 `GOADMIN_CONFIG_DIR=server/config`
- 设置 `GOMODCACHE=server/.cache/go-mod`
- 设置 `GOCACHE=server/.cache/go-build`
