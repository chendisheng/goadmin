...
type ApplyExecutionRule struct {
	RequireManifest           bool     `json:"require_manifest,omitempty"`
	RequireGeneratedBootstrap bool     `json:"require_generated_bootstrap,omitempty"`
	RequireInstall            bool     `json:"require_install,omitempty"`
	RequirePolicySync         bool     `json:"require_policy_sync,omitempty"`
	RequireValidation         bool     `json:"require_validation,omitempty"`
	AllowPartial              bool     `json:"allow_partial,omitempty"`
	AllowSharedResources      bool     `json:"allow_shared_resources,omitempty"`
	AllowPolicyFallback       bool     `json:"allow_policy_fallback,omitempty"`
	Notes                     []string `json:"notes,omitempty"`
}

func (r ApplyExecutionRule) Normalize() ApplyExecutionRule {
	r.Notes = normalizeUniqueStringSlice(r.Notes)
	return r
}

type ApplyDeletePairing struct {
	...
	NonSymmetricNotes []string `json:"non_symmetric_notes,omitempty"`
}

func DefaultApplyDeletePairing() ApplyDeletePairing {
	return ApplyDeletePairing{
		Operation:     "apply",
		Counterpart:   "delete",
		SourceOfTruth: "manifest.yaml + ownership metadata + execution result",
		ApplyScope: []string{
			"generate source assets",
			"write manifest.yaml",
			"sync policy.csv",
			"install runtime menus",
			"install runtime routes",
			"install runtime permissions",
			"refresh runtime caches",
		},
		DeleteScope: []string{
			"remove generated source assets",
			"refresh bootstrap registry",
			"remove runtime menus",
			"remove runtime routes",
		...
		SharedScope: []string{
			...
			"shared policy entries",
		},
		NonSymmetricNotes: []string{
			"apply may create assets that delete only reclaims when CodeGen owns them",
			"delete skips shared or manual resources instead of reversing all apply side effects",
			"policy sync follows policy.csv and is not treated as a reversible database source of truth",
		},
	}.Normalize()
}

func (p ApplyDeletePairing) Normalize() ApplyDeletePairing {
	p.Operation = strings.TrimSpace(p.Operation)
	p.Counterpart = strings.TrimSpace(p.Counterpart)
	p.SourceOfTruth = strings.TrimSpace(p.SourceOfTruth)
	p.ApplyScope = normalizeUniqueStringSlice(p.ApplyScope)
	p.DeleteScope = normalizeUniqueStringSlice(p.DeleteScope)
	p.SharedScope = normalizeUniqueStringSlice(p.SharedScope)
	p.NonSymmetricNotes = normalizeUniqueStringSlice(p.NonSymmetricNotes)
	if p.Operation == "" {
		p.Operation = "apply"
	}
	if p.Counterpart == "" {
		p.Counterpart = "delete"
	}
	return p
}
...
const (
	ApplyStagePrepare    ApplyStage = "prepare"
	ApplyStageGenerate   ApplyStage = "generate"
	ApplyStageInstall    ApplyStage = "install"
	ApplyStagePolicySync ApplyStage = "policy_sync"
...
const (
	ApplyStatusUnknown       ApplyStatus = ""
	ApplyStatusPlanned       ApplyStatus = "planned"
	ApplyStatusDryRun        ApplyStatus = "dry_run"
	ApplyStatusGenerating    ApplyStatus = "generating"
	ApplyStatusInstalling    ApplyStatus = "installing"
	ApplyStatusPolicySyncing ApplyStatus = "policy_syncing"
...
func (s ApplyStatus) IsTerminal() bool {
	switch s {
	case ApplyStatusSucceeded, ApplyStatusPartial, ApplyStatusFailed:
		return true
	default:
		return false
	}
...
type ApplyFailureCategory string

const (
	ApplyFailureCategoryUnknown    ApplyFailureCategory = ""
	ApplyFailureCategoryGeneration ApplyFailureCategory = "generation"
	ApplyFailureCategoryInstall    ApplyFailureCategory = "install"
	ApplyFailureCategoryPolicy     ApplyFailureCategory = "policy"
	ApplyFailureCategoryValidation ApplyFailureCategory = "validation"
)

type ApplyFailureStage string

const (
	ApplyFailureStageUnknown    ApplyFailureStage = ""
	ApplyFailureStagePrepare    ApplyFailureStage = "prepare"
	ApplyFailureStageGenerate   ApplyFailureStage = "generate"
	ApplyFailureStageInstall    ApplyFailureStage = "install"
	ApplyFailureStagePolicySync ApplyFailureStage = "policy_sync"
	ApplyFailureStageValidate   ApplyFailureStage = "validate"
)
...
type ApplyItem struct {
	...
	Store    PolicyStoreKind `json:"store,omitempty"`
	Stage    ApplyStage      `json:"stage,omitempty"`
...
type ApplyPlanSummary struct {
	...
	PolicySync   int `json:"policy_sync,omitempty"`
...
type ApplyValidationIssue struct {
	Item     ApplyItem      `json:"item,omitempty"`
	Stage    ApplyStage     `json:"stage,omitempty"`
	Message  string         `json:"message,omitempty"`
...
type ApplyGenerationReport struct {
	StartedAt  time.Time       `json:"started_at,omitempty"`
	FinishedAt time.Time       `json:"finished_at,omitempty"`
	Status     string          `json:"status,omitempty"`
	Created    []ApplyItem     `json:"created,omitempty"`
	Updated    []ApplyItem     `json:"updated,omitempty"`
	Skipped    []ApplyItem     `json:"skipped,omitempty"`
	Warnings   []string        `json:"warnings,omitempty"`
	Conflicts  []ApplyConflict `json:"conflicts,omitempty"`
}

type ApplyInstallationReport struct {
	StartedAt  time.Time       `json:"started_at,omitempty"`
	FinishedAt time.Time       `json:"finished_at,omitempty"`
	Status     string          `json:"status,omitempty"`
	Created    []ApplyItem     `json:"created,omitempty"`
...
type ApplyPolicySyncReport struct {
	StartedAt  time.Time       `json:"started_at,omitempty"`
	FinishedAt time.Time       `json:"finished_at,omitempty"`
	Status     string          `json:"status,omitempty"`
	Store      PolicyStoreKind `json:"store,omitempty"`
	Synced     []ApplyItem     `json:"synced,omitempty"`
	Skipped    []ApplyItem     `json:"skipped,omitempty"`
	Warnings   []string        `json:"warnings,omitempty"`
	Conflicts  []ApplyConflict `json:"conflicts,omitempty"`
}

type ApplyValidationReport struct {
	StartedAt  time.Time              `json:"started_at,omitempty"`
	FinishedAt time.Time              `json:"finished_at,omitempty"`
	Status     string                 `json:"status,omitempty"`
	Verified   bool                   `json:"verified,omitempty"`
	Checked    int                    `json:"checked,omitempty"`
	Issues     []ApplyValidationIssue `json:"issues,omitempty"`
}

type ApplyFailure struct {
	Item        ApplyItem            `json:"item,omitempty"`
	Category    ApplyFailureCategory `json:"category,omitempty"`
	Stage       ApplyFailureStage    `json:"stage,omitempty"`
...
type ApplyPlan struct {
	Request      ApplyRequest           `json:"request,omitempty"`
	Module       string                 `json:"module,omitempty"`
	Kind         string                 `json:"kind,omitempty"`
	DryRun       bool                   `json:"dry_run,omitempty"`
	Force        bool                   `json:"force,omitempty"`
	PolicyStore  PolicyStoreKind        `json:"policy_store,omitempty"`
	Generation   []ApplyItem            `json:"generation,omitempty"`
	Installation []ApplyItem            `json:"installation,omitempty"`
	PolicySync   []ApplyItem            `json:"policy_sync,omitempty"`
	Validation   []ApplyValidationIssue `json:"validation,omitempty"`
	Warnings     []string               `json:"warnings,omitempty"`
	Conflicts    []ApplyConflict        `json:"conflicts,omitempty"`
	Pairing      ApplyDeletePairing     `json:"pairing,omitempty"`
	Summary      ApplyPlanSummary       `json:"summary,omitempty"`
}

type ApplyResultSummary struct {
	Generated        int   `json:"generated,omitempty"`
	Installed        int   `json:"installed,omitempty"`
	PolicySynced     int   `json:"policy_synced,omitempty"`
	ValidationChecks int   `json:"validation_checks,omitempty"`
	ValidationIssues int   `json:"validation_issues,omitempty"`
	Skipped          int   `json:"skipped,omitempty"`
	Failed           int   `json:"failed,omitempty"`
	TotalApplied     int   `json:"total_applied,omitempty"`
...
type ApplyResult struct {
	...
	StartedAt    time.Time               `json:"started_at,omitempty"`
	FinishedAt   time.Time               `json:"finished_at,omitempty"`
	Generation   ApplyGenerationReport   `json:"generation,omitempty"`
	Installation ApplyInstallationReport `json:"installation,omitempty"`
	PolicySync   ApplyPolicySyncReport   `json:"policy_sync,omitempty"`
	Validation   ApplyValidationReport   `json:"validation,omitempty"`
	Failures     []ApplyFailure          `json:"failures,omitempty"`
	Warnings     []string                `json:"warnings,omitempty"`
	Pairing      ApplyDeletePairing      `json:"pairing,omitempty"`
	Summary      ApplyResultSummary      `json:"summary,omitempty"`
}
