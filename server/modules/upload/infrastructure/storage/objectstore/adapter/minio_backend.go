...
func newMinIOServer(cfg Config) (minioServer, bool, error) {
	...
	if err := server.ensureBucket(context.Background()); err != nil {
		return nil, false, err
	}
...
func (c *minioObjectServer) Name() string {
	if c == nil {
		return ""
	}
	return "minio"
...
func (c *minioObjectServer) PutObject(ctx context.Context, req storagecontract.PutObjectRequest) (*storagecontract.PutObjectResult, error) {
	if c == nil || c.client == nil {
		return nil, apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
	if req.Reader == nil {
...
func (c *minioObjectServer) GetObject(ctx context.Context, key string) (io.ReadCloser, *storagecontract.ObjectInfo, error) {
	if c == nil || c.client == nil {
		return nil, nil, apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
...
func (c *minioObjectServer) HeadObject(ctx context.Context, key string) (*storagecontract.ObjectInfo, error) {
	if c == nil || c.client == nil {
		return nil, apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
	key, err := normalizeKey(key)
...
func (c *minioObjectServer) DeleteObject(ctx context.Context, key string) error {
	if c == nil || c.client == nil {
		return apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
	key, err := normalizeKey(key)
	if err != nil {
		return err
...
func (c *minioObjectServer) Exists(ctx context.Context, key string) (bool, error) {
	if c == nil || c.client == nil {
		return false, apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
	key, err := normalizeKey(key)
	if err != nil {
		return false, err
...
func (c *minioObjectServer) PublicURL(ctx context.Context, key string) (string, error) {
	if c == nil || c.client == nil {
		return "", apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
...
func (c *minioObjectServer) SignedURL(ctx context.Context, key string, opts storagecontract.SignedURLOptions) (string, error) {
	if c == nil || c.client == nil {
		return "", apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
	if strings.TrimSpace(c.publicBaseURL) == "" {
	...
	case "PUT":
		...
		return presigned.String(), nil
	default:
		return "", apperrors.NewWithKey(apperrors.CodeBadRequest, "upload.minio_signed_url_method_unsupported", fmt.Sprintf("unsupported signed url method %q for minio server", opts.Method))
	}
}

func (c *minioObjectServer) ensureBucket(ctx context.Context) error {
	if c == nil || c.client == nil {
		return apperrors.NewWithKey(apperrors.CodeInternal, "upload.minio_server_not_configured", "minio server is not configured")
	}
...
func (c *minioObjectServer) objectInfoFromMinIO(info minio.ObjectInfo, key string) *storagecontract.ObjectInfo {
...
func (c *minioObjectServer) buildPublicURL(key string) string {
...
func (c *minioObjectServer) storeMetadata(key string, metadata map[string]string) {
	if c == nil {
		return
	}
...
func (c *minioObjectServer) cachedMetadata(key string) map[string]string {
	if c == nil {
...
func (c *minioObjectServer) deleteMetadata(key string) {
	if c == nil {
		return
	}
	key = strings.TrimSpace(key)
