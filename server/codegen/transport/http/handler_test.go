...
func createDeleteModuleFixture(t *testing.T, root, module string, includeManifest, includeUnknown bool) {
	...
	mustMkdirAllHTTP(t, filepath.Join(root, "server", "core", "bootstrap"))
...
`))
	...
	writeFixtureHTTP(t, filepath.Join(root, "server", "core", "bootstrap", "modules_gen.go"), "package bootstrap\n\nimport (\n\t\"goadmin/modules/"+module+"\"\n)\n\nfunc generatedModules() []Module {\n\treturn []Module{\n\t\t"+module+".NewBootstrap(),\n\t}\n}\n")
	writeFixtureHTTP(t, filepath.Join(root, "server", "core", "bootstrap", "modules_builtin.go"), "package bootstrap\n\nimport ()\n\nfunc builtinModules() []Module {\n\treturn []Module{}\n}\n")
...
func TestHandlerInstallManifest(t *testing.T) {
	...
	root := t.TempDir()
	manifestDir := filepath.Join(root, "server", "modules", "book")
...
func TestHandlerGenerateDatabase(t *testing.T) {
	...
	assertExists(t, filepath.Join(root, "server", "modules", "book", "module.go"))
