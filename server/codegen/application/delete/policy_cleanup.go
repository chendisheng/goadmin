...
func NewPolicyCleanupService(deps PolicyCleanupDependencies) (*PolicyCleanupService, error) {
	serverRoot := strings.TrimSpace(deps.ServerRoot)
	if serverRoot == "" {
		serverRoot = resolveServerRoot(deps.ProjectRoot)
	}
	policyPath := strings.TrimSpace(deps.PolicyPath)
	if policyPath == "" {
		policyPath = filepath.Join(serverRoot, "core", "auth", "casbin", "adapter", "policy.csv")
	}
