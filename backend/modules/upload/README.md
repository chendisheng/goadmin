...
上传模块配置位于 `backend/config/config.yaml` 对应的 `upload.storage.*` 结构中。
...
`backend/modules/upload/bootstrap.go`
...
`backend/modules/upload/application/service/service.go`
...
`backend/modules/upload/transport/http/router.go`
...
- `backend/modules/upload/application/service/service_test.go`
  - 上传成功与删除清理
  - 不允许的扩展名拒绝
- `backend/modules/upload/domain/model/file_asset_test.go`
  - 表名与克隆行为
  - 状态枚举常量
- `backend/modules/upload/module_test.go`
  - 模块元数据
  - manifest 路由 / 菜单 / 权限校验
  - HTTP 路由注册回归
- `backend/core/auth/casbin/service/casbin_service_test.go`
