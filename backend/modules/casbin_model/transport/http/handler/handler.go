...
func (h *Handler) List(c coretransport.Context) {
	...
	if err := c.ShouldBindQuery(&req); err != nil {
		status, body := response.Failure(coreerrors.WrapWithKey(err, coreerrors.CodeBadRequest, "casbin_model.invalid_list_request", "invalid list request"), requestID(c))
		c.JSON(status, body)
		return
	}
...
func (h *Handler) Create(c coretransport.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		status, body := response.Failure(coreerrors.WrapWithKey(err, coreerrors.CodeBadRequest, "casbin_model.invalid_create_request", "invalid create request"), requestID(c))
		c.JSON(status, body)
		return
	}
...
func (h *Handler) Update(c coretransport.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		status, body := response.Failure(coreerrors.WrapWithKey(err, coreerrors.CodeBadRequest, "casbin_model.invalid_update_request", "invalid update request"), requestID(c))
		c.JSON(status, body)
		return
	}
