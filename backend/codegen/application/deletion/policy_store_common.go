...
func buildPolicyCleanupPreview(req PolicyCleanupRequest, store deletionmodel.PolicyStoreKind, rules []deletionmodel.PolicyAsset) PolicyCleanupPreview {
	...
	for _, selector := range req.Selectors {
		...
		if item.Decision == "conflict" {
			preview.Conflicts = append(preview.Conflicts, deletionmodel.DeleteConflict{
...
func validatePolicyStoreRules(ctx context.Context, load func(context.Context) ([]deletionmodel.PolicyAsset, error)) error {
	if load == nil {
...
func policyAssetsByModule(assets []deletionmodel.PolicyAsset, module string) []deletionmodel.PolicyAsset {
	module = strings.TrimSpace(module)
	if module == "" || len(assets) == 0 {
		return assets
	}
	filtered := make([]deletionmodel.PolicyAsset, 0, len(assets))
...
func policyAssetModule(asset deletionmodel.PolicyAsset) string {
...
func policyAssetSourceRef(asset deletionmodel.PolicyAsset) string {
...
func policyAssetManaged(asset deletionmodel.PolicyAsset) bool {
	if asset.Managed {
		return true
	}
	if asset.Metadata == nil {
...
func previewPolicySelector(selector deletionmodel.PolicySelector, module string, rules []deletionmodel.PolicyAsset) PolicyCleanupItem {
	...
	matches := make([]deletionmodel.PolicyAsset, 0)
...
func findMatchingSelector(req PolicyCleanupRequest, rule deletionmodel.PolicyAsset) *deletionmodel.PolicySelector {
...
func verifyPolicySelections(rules []deletionmodel.PolicyAsset, selectors []deletionmodel.PolicySelector) bool {
...
func policySelectorKey(selector deletionmodel.PolicySelector) string {
	parts := []string{
		string(choosePolicyStoreKind(selector.Store, deletionmodel.PolicyStoreUnknown)),
...
func selectorToSummary(selector deletionmodel.PolicySelector) string {
	parts := []string{
		strings.TrimSpace(selector.PType),
		strings.TrimSpace(selector.V0),
		strings.TrimSpace(selector.V1),
...
func selectorManaged(selector deletionmodel.PolicySelector) bool {
...
func selectorModule(selector deletionmodel.PolicySelector) string {
...
func selectorMatchesModule(selector deletionmodel.PolicySelector, module string) bool {
	module = strings.TrimSpace(module)
	if module == "" {
		return true
	}
...
func selectorToPolicyAsset(selector deletionmodel.PolicySelector, sourceRef string) deletionmodel.PolicyAsset {
...
func ruleMatchesSelector(rule deletionmodel.PolicyAsset, selector deletionmodel.PolicySelector) bool {
