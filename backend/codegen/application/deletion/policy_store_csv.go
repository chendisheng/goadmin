...
func (s *CSVPolicyStore) Kind() deletionmodel.PolicyStoreKind {
	return deletionmodel.PolicyStoreCSV
}

func (s *CSVPolicyStore) ListByModule(ctx context.Context, module string) ([]deletionmodel.PolicyAsset, error) {
	if s == nil {
...
func (s *CSVPolicyStore) DeleteBySelector(ctx context.Context, selector deletionmodel.PolicySelector) (int, error) {
	if s == nil {
	...
	filtered := make([]deletionmodel.PolicyAsset, 0, len(rules))
	deleted := 0
...
func (s *CSVPolicyStore) Validate(ctx context.Context) error {
	...
	return validatePolicyStoreRules(ctx, func(ctx context.Context) ([]deletionmodel.PolicyAsset, error) {
		_ = ctx
...
func loadCSVRules(path string) ([]deletionmodel.PolicyAsset, error) {
	...
	defer file.Close()
	var rules []deletionmodel.PolicyAsset
...
func writeCSVRulesAtomic(path string, rules []deletionmodel.PolicyAsset) error {
...
func normalizeCSVRule(parts []string) (deletionmodel.PolicyAsset, error) {
	...
	if len(trimmed) < 3 || len(trimmed) > 6 {
		return deletionmodel.PolicyAsset{}, fmt.Errorf("invalid policy row")
	}
	asset := deletionmodel.PolicyAsset{Store: deletionmodel.PolicyStoreCSV, PType: "p", V0: trimmed[0], V1: trimmed[1], V2: trimmed[2]}
	if len(trimmed) > 3 {
		asset.V3 = trimmed[3]
	}
	if len(trimmed) > 4 {
...
func formatCSVRuleLine(rule deletionmodel.PolicyAsset) string {
	values := []string{
