...
func (s *DBPolicyStore) Kind() deletionmodel.PolicyStoreKind {
	return deletionmodel.PolicyStoreDB
}

func (s *DBPolicyStore) ListByModule(ctx context.Context, module string) ([]deletionmodel.PolicyAsset, error) {
	if s == nil || s.db == nil {
...
func (s *DBPolicyStore) DeleteBySelector(ctx context.Context, selector deletionmodel.PolicySelector) (int, error) {
	if s == nil || s.db == nil {
...
func loadDBRules(ctx context.Context, db *gorm.DB) ([]deletionmodel.PolicyAsset, error) {
	if db == nil {
		return nil, fmt.Errorf("db policy store requires db")
	}
	...
	rules := make([]deletionmodel.PolicyAsset, 0, len(records))
	for _, record := range records {
		if strings.TrimSpace(record.V0) == "" || strings.TrimSpace(record.V1) == "" || strings.TrimSpace(record.V2) == "" {
			continue
		}
		rules = append(rules, deletionmodel.PolicyAsset{
...
func buildDBSelectorQuery(selector deletionmodel.PolicySelector) map[string]any {
...
func verifyPolicySelectionsDB(rules []deletionmodel.PolicyAsset, selectors []deletionmodel.PolicySelector) bool {
...
func selectorReason(selector deletionmodel.PolicySelector, module string) (string, bool) {
