...
func (s *Service) runInstall(ctx context.Context, workload Workload) (installResult installapp.InstallResult, err error) {
	...
	if manifestPath == "" {
		manifestPath = filepath.Join(s.projectRoot, "backend", "modules", workload.Request.Module, "manifest.yaml")
...
func normalizeWorkload(workload Workload) Workload {
	...
	if workload.ManifestPath == "" && workload.Manifest.Module != "" {
		workload.ManifestPath = filepath.Join("backend", "modules", workload.Manifest.Module, "manifest.yaml")
	}
	return workload
}
