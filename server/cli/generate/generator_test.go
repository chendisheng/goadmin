...
func TestGenerateModule(t *testing.T) {
	...
	modulePath := filepath.Join(root, "server", "modules", "inventory", "module.go")
	manifestPath := filepath.Join(root, "server", "modules", "inventory", "manifest.yaml")
	localeZhPath := filepath.Join(root, "server", "modules", "inventory", "locales", "zh-CN", "inventory.yaml")
	localeEnPath := filepath.Join(root, "server", "modules", "inventory", "locales", "en-US", "inventory.yaml")
	...
	if _, err := os.Stat(filepath.Join(root, "server", "modules", "inventory", "application")); !os.IsNotExist(err) {
...
func TestGenerateCRUDAndPolicyDedup(t *testing.T) {
	...
	modelPath := filepath.Join(root, "server", "modules", "article", "domain", "model", "article.go")
	bootstrapPath := filepath.Join(root, "server", "modules", "article", "bootstrap.go")
	schemaPath := filepath.Join(root, "server", "modules", "article", "schema.sql")
	requestPath := filepath.Join(root, "server", "modules", "article", "transport", "http", "request", "article.go")
	responsePath := filepath.Join(root, "server", "modules", "article", "transport", "http", "response", "article.go")
	localeZhPath := filepath.Join(root, "server", "modules", "article", "locales", "zh-CN", "article.yaml")
	localeEnPath := filepath.Join(root, "server", "modules", "article", "locales", "en-US", "article.yaml")
	policyPath := filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv")
	...
	repoPath := filepath.Join(root, "server", "modules", "article", "infrastructure", "repo", "gorm.go")
...
func TestGenerateCRUDPrimaryKeyModes(t *testing.T) {
	...
	stringModelPath := filepath.Join(root, "server", "modules", "article", "domain", "model", "article.go")
	stringRepoPath := filepath.Join(root, "server", "modules", "article", "infrastructure", "repo", "gorm.go")
	...
	numericModelPath := filepath.Join(root, "server", "modules", "chapter", "domain", "model", "chapter.go")
	numericRepoPath := filepath.Join(root, "server", "modules", "chapter", "infrastructure", "repo", "gorm.go")
...
func TestRefreshBootstrapRegistryFiltersGeneratedModules(t *testing.T) {
	...
	writeBootstrap := func(name, content string) {
		path := filepath.Join(root, "server", "modules", name, "bootstrap.go")
...
func TestGenerateManifestRendersMenuParentPath(t *testing.T) {
	...
	manifestPath := filepath.Join(root, "server", "modules", "book", "manifest.yaml")
...
func TestGenerateCRUDFrontendPathsUseRepoWebRoot(t *testing.T) {
	...
	assertPathNotExists(t, filepath.Join(root, "server", "web", "src", "api", "article.ts"))
	assertPathNotExists(t, filepath.Join(root, "server", "web", "src", "views", "article", "index.vue"))
...
func TestGenerateCRUDFrontendRendersUsablePage(t *testing.T) {
	...
	localeZhPath := filepath.Join(root, "web", "src", "i18n", "locales", "zh-CN", "book.json")
	localeEnPath := filepath.Join(root, "web", "src", "i18n", "locales", "en-US", "book.json")
	assertFileContains(t, apiPath, "const basePath = '/books'")
	assertFileContains(t, apiPath, "import type { ListResponse } from '@/types/admin'")
	assertFileContains(t, apiPath, "export interface BookListQuery")
	assertFileContains(t, apiPath, "export interface BookItem")
	assertFileContains(t, apiPath, "http.get<ListResponse<BookItem>>(basePath, { params })")
	assertFileContains(t, apiPath, "http.post<BookItem>(basePath, data)")
	assertFileContains(t, apiPath, "http.put<BookItem>(basePath + '/' + id, data)")
	assertFileContains(t, apiPath, "http.delete<{ deleted: boolean }>(basePath + '/' + id)")
	assertFileContains(t, routePath, "titleKey:")
	assertFileContains(t, routePath, "titleDefault:")
	...
	assertPathExists(t, localeZhPath)
	assertPathExists(t, localeEnPath)
	assertFileContains(t, localeZhPath, "book.page.description")
	assertFileContains(t, localeZhPath, "book.field.title.label")
	assertFileContains(t, localeZhPath, "book.field.title.placeholder")
	assertFileContains(t, localeEnPath, "book.page.description")
	assertFileContains(t, localeEnPath, "book.field.title.label")
	assertFileContains(t, localeEnPath, "book.field.title.placeholder")

	assertFileContains(t, viewPath, "useAppI18n")
	assertFileContains(t, viewPath, "book.page.description")
	assertFileContains(t, viewPath, "book.field.title.label")
	assertFileContains(t, viewPath, "book.field.title.placeholder")
	assertFileContains(t, viewPath, "common.id")
	assertFileContains(t, viewPath, "common.created_at")
	assertFileContains(t, viewPath, "common.updated_at")
	assertFileContains(t, viewPath, "save_failed")
	assertFileContains(t, viewPath, "delete_confirm")
...
func TestGeneratePageUsesRepoWebRoot(t *testing.T) {
	...
	assertFileContains(t, routePath, "titleKey:")
	assertFileContains(t, routePath, "titleDefault:")
	assertPathNotExists(t, filepath.Join(root, "server", "web", "src", "views", "system", "report.vue"))
	assertPathNotExists(t, filepath.Join(root, "server", "web", "src", "router", "modules", "system-report.ts"))
}
...
func TestGenerateCRUDPreservesManualGoChanges(t *testing.T) {
	...
	routerPath := filepath.Join(root, "server", "modules", "book", "transport", "http", "router.go")
...
func TestGeneratePlugin(t *testing.T) {
	...
	pluginPath := filepath.Join(root, "server", "plugin", "builtin", "demo", "demo.go")
	localeZhPath := filepath.Join(root, "server", "plugin", "builtin", "demo", "locales", "zh-CN", "plugin.yaml")
	localeEnPath := filepath.Join(root, "server", "plugin", "builtin", "demo", "locales", "en-US", "plugin.yaml")
	assertFileContains(t, pluginPath, "package demo")
	assertFileContains(t, pluginPath, "pong from demo plugin")
	assertFileContains(t, filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "p, admin, /plugins/demo/ping, GET")
