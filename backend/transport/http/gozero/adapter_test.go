...
func TestContextWrapperDelegates(t *testing.T) {
	...
	base := &fakeContext{method: "GET", path: "/health", headers: map[string]string{"X": "1"}}
	ctx := NewContext(base)
	if got := ctx.Method(); got != "GET" {
		t.Fatalf("Method() = %q, want GET", got)
	}
	if got := ctx.Path(); got != "/health" {
		t.Fatalf("Path() = %q, want /health", got)
	}
	if got := ctx.Header("X"); got != "1" {
		t.Fatalf("Header(X) = %q, want 1", got)
	}
...
func TestRouterWrapperDelegates(t *testing.T) {
	...
	group.GET("/ping", func(coretransport.Context) {})

	if len(base.groups) != 1 || base.groups[0] != "/v1" {
		t.Fatalf("groups = %v, want [/v1]", base.groups)
	}
	if len(base.routes) != 1 || base.routes[0] != "GET /ping" {
		t.Fatalf("routes = %v, want [GET /ping]", base.routes)
	}
}
