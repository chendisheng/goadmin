...
func runRemovePreview(root string, deletion *deleteapp.Service, args []string) error {
	...
	policyStore := fs.String("policy-store", "", "policy store server (csv or db)")
	fs.SetOutput(os.Stderr)
	if err := fs.Parse(args); err != nil {
		return err
	}
...
func describeSchemaResourceActions(resource schema.Resource) []string {
	...
	if pluginName := choosePluginName(resource); pluginName != "" && (resource.Kind == schema.KindPlugin || resource.Kind == schema.KindServerPlugin) {
...
func shouldSkipManifest(resource schema.Resource) bool {
	switch resource.Kind {
	case schema.KindPlugin, schema.KindServerPlugin, schema.KindConfig:
		return true
	default:
		return false
	}
