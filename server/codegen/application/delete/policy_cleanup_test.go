...
func TestPolicyCleanupRequestNormalizeAndBuildRequest(t *testing.T) {
	...
	req := PolicyCleanupRequest{
		Module: " book ",
		Store:  lifecycle.PolicyStoreUnknown,
		Selectors: []lifecycle.PolicySelector{
			{
				Store:    lifecycle.PolicyStoreUnknown,
				Module:   " book ",
				PType:    " p ",
	...
	normalized := req.Normalize(lifecycle.PolicyStoreCSV)
	if normalized.Module != "book" {
		t.Fatalf("normalized module = %q, want book", normalized.Module)
	}
	if normalized.Store != lifecycle.PolicyStoreCSV {
	...
	plan := lifecycle.DeletePlan{
		Module:      "book",
		PolicyStore: lifecycle.PolicyStoreCSV,
		PolicyChanges: []lifecycle.DeleteItem{
			{Selector: &lifecycle.PolicySelector{Store: lifecycle.PolicyStoreCSV, Module: "book", Metadata: map[string]any{"managed": true}, PType: "p", V0: "book", V1: "book", V2: "read"}},
			{Selector: nil},
		},
	}
	cleanupReq := BuildPolicyCleanupRequest(plan)
	if cleanupReq.Store != lifecycle.PolicyStoreCSV {
...
func TestCSVPolicyStoreDeleteRemovesMatchedRuleAndValidates(t *testing.T) {
	...
	result, err := store.Delete(context.Background(), PolicyCleanupRequest{
		Module: "book",
		Selectors: []lifecycle.PolicySelector{{
			Store:    lifecycle.PolicyStoreCSV,
			Module:   "book",
			PType:    "p",
			V0:       "book",
			V1:       "book",
...
func TestDBPolicyStorePreviewDeleteAndValidate(t *testing.T) {
	...
	if preview.Store != lifecycle.PolicyStoreDB {
...
func containsDBRule(rules []lifecycle.PolicyAsset, v0, v1, v2 string) bool {
