...
func NewService(deps Dependencies) *Service {
	...
	serverRoot := filepath.Clean(strings.TrimSpace(deps.ServerRoot))
	if serverRoot == "" || serverRoot == "." {
		serverRoot = resolveServerRoot(projectRoot)
	}
	return &Service{
...
func (s *Service) resolveModule(moduleName string, req lifecycle.DeleteRequest) (ModuleResolution, error) {
	moduleDir, serverRoot, err := s.locateModuleDir(moduleName)
	if err != nil {
	...
	registryPath := filepath.Join(serverRoot, "core", "bootstrap", "modules_gen.go")
	builtinRegistryPath := filepath.Join(serverRoot, "core", "bootstrap", "modules_builtin.go")
	...
	resolved := ModuleResolution{
		...
		ServerRoot:          displayPath(displayRoot, serverRoot),
...
func (s *Service) buildPlan(req lifecycle.DeleteRequest, resolution ModuleResolution) (lifecycle.DeletePlan, error) {
	...
	manifestIndex, indexErr := loadModuleManifestReferenceIndex(s.serverRoot)
	if indexErr != nil {
...
func (s *Service) collectFrontendCandidates(module string, managed bool, displayRoot string) []lifecycle.DeleteItem {
	paths := []struct {
		...
		{path: filepath.Join(s.serverRoot, "..", "web", "src", "api", module+".ts"), kind: lifecycle.AssetKindFrontendFile},
		{path: filepath.Join(s.serverRoot, "..", "web", "src", "router", "modules", module+".ts"), kind: lifecycle.AssetKindFrontendFile},
		{path: filepath.Join(s.serverRoot, "..", "web", "src", "views", module, "index.vue"), kind: lifecycle.AssetKindFrontendFile},
	}
...
func (s *Service) locateModuleDir(module string) (string, string, error) {
	candidates := []string{
		filepath.Join(s.serverRoot, "modules", module),
		filepath.Join(s.projectRoot, "server", "modules", module),
	...
	for _, candidate := range candidates {
		...
		if info, err := os.Stat(candidate); err == nil && info.IsDir() {
			...
			if serverRoot == "" {
				serverRoot = resolveServerRoot(s.projectRoot)
			}
			return candidate, serverRoot, nil
		}
	}
	return "", "", fmt.Errorf("module %q not found under server/modules", module)
}
...
func resolveServerRoot(projectRoot string) string {
	projectRoot = filepath.Clean(strings.TrimSpace(projectRoot))
	if projectRoot == "" || projectRoot == "." {
		return "."
	}
	serverCandidate := filepath.Join(projectRoot, "server")
	if info, err := os.Stat(filepath.Join(serverCandidate, "modules")); err == nil && info.IsDir() {
...
func (s *Service) executeDeletePlan(ctx context.Context, plan lifecycle.DeletePlan) ([]lifecycle.DeleteItem, []lifecycle.DeleteItem, []lifecycle.DeleteFailure, []string) {
	...
	for _, item := range plan.SourceFiles {
		if item.Kind == lifecycle.AssetKindSourceDirectory {
			if err := s.cleanupEmptyParents(s.resolveExecutionPath(item.Path), filepath.Join(s.serverRoot, "modules")); err != nil {
...
func (s *Service) cleanupGeneratedAncestors(displayPathValue string) error {
	...
	stopAt := filepath.Join(s.serverRoot, "modules")
...
func (s *Service) refreshBootstrapRegistry() error {
	...
	modulesDir := filepath.Join(s.serverRoot, "modules")
	...
	registryPath := filepath.Join(s.serverRoot, "core", "bootstrap", "modules_gen.go")
