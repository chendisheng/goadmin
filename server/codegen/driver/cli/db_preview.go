...
func previewSchemaResourceFiles(root string, resource schema.Resource, force bool) []DatabasePreviewFile {
	...
	case schema.KindModule, schema.KindBusinessModule, schema.KindServerModule:
		if moduleName := chooseModuleName(resource); moduleName != "" {
			add(filepath.Join(root, "server", "modules", moduleName, "module.go"), "module", "create", "module scaffold", moduleName)
			add(filepath.Join(root, "server", "modules", moduleName, "manifest.yaml"), "manifest", "create", "module manifest", moduleName)
		}
		if hasCRUDContent(resource) {
			files = append(files, previewCRUDFiles(root, resource, force)...)
		}
	case schema.KindCRUD, schema.KindServerCRUD:
		files = append(files, previewCRUDFiles(root, resource, force)...)
	case schema.KindPlugin, schema.KindServerPlugin:
		if pluginName := choosePluginName(resource); pluginName != "" {
			add(filepath.Join(root, "server", "plugin", "builtin", pluginName, pluginName+".go"), "plugin", "create", "plugin scaffold", pluginName)
			add(filepath.Join(root, "server", "web", "src", "plugins", pluginName+".ts"), "plugin-frontend", "create", "plugin frontend entry", pluginName)
			add(filepath.Join(root, "server", "web", "src", "views", "plugin", pluginName, "index.vue"), "plugin-view", "create", "plugin view", pluginName)
			add(filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "policy", "append", "plugin ping policy", pluginName)
		}
	...
	case schema.KindConfig:
		...
		add(filepath.Join(root, "server", "config", "config."+schema.NormalizeName(name)+".yaml"), "config", "create", "config profile", name)
	...
	if len(pages) > 0 {
		...
		for _, page := range pages {
			...
			add(filepath.Join(root, "server", "web", "src", "views", scope, pageSlug+".vue"), "page-view", "create", page.Title(), scope)
			add(filepath.Join(root, "server", "web", "src", "router", "modules", scope+"-"+pageSlug+".ts"), "page-router", "create", page.Title(), scope)
		}
		add(filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "policy", "append", "page policies", resourceName)
	}
	if len(resource.Routes) > 0 {
		add(filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "policy", "append", "route policies", resourceName)
	}
	if len(resource.Permissions) > 0 {
		add(filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "policy", "append", "permission policies", resourceName)
	}
...
func previewCRUDFiles(root string, resource schema.Resource, force bool) []DatabasePreviewFile {
	...
	base := filepath.Join(root, "server", "modules", entityLower)
	...
	if resource.GenerateFrontend || len(resource.Pages) > 0 || strings.TrimSpace(resource.Framework.Frontend) != "" {
		files = append(files,
			previewFile(filepath.Join(root, "server", "web", "src", "api", entityLower+".ts"), "frontend-api", "create", "frontend api", entityLower, force, false),
			previewFile(filepath.Join(root, "server", "web", "src", "router", "modules", entityLower+".ts"), "frontend-router", "create", "frontend router", entityLower, force, false),
			previewFile(filepath.Join(root, "server", "web", "src", "views", entityLower, "index.vue"), "frontend-view", "create", "frontend view", entityLower, force, false),
		)
	}
	if resource.GeneratePolicy || len(resource.Routes) > 0 || len(resource.Pages) > 0 || len(resource.Permissions) > 0 {
		files = append(files, previewFile(filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "policy", "append", "CRUD policy lines", entityLower, force, true))
	}
	return sortDatabasePreviewFiles(files)
}
