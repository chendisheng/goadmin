...
`)
	...
	crudModelPath := filepath.Join(root, "server", "modules", "item", "domain", "model", "item.go")
...
func TestRunRemovePreview(t *testing.T) {
	...
	root := t.TempDir()
	deletionRoot := filepath.Join(root, "server")
...
func TestExecuteDSLResourcesPreservesSchemaSQLFieldComments(t *testing.T) {
	...
	schemaPath := filepath.Join(root, "server", "modules", "order", "schema.sql")
...
func TestRunGenerateDBGenerate(t *testing.T) {
	...
	assertFileExists(t, filepath.Join(root, "server", "modules", "book", "module.go"))
	assertFileExists(t, filepath.Join(root, "web", "src", "views", "book", "index.vue"))
	assertFileContains(t, filepath.Join(root, "server", "modules", "book", "manifest.yaml"), "menus:")
	assertFileContains(t, filepath.Join(root, "server", "modules", "book", "manifest.yaml"), "path: /books")
	assertFileContains(t, filepath.Join(root, "server", "core", "auth", "casbin", "adapter", "policy.csv"), "p, admin, /api/v1/books, GET")
}
...
func TestExecuteDatabaseDocumentDryRunReport(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "server", "modules", "book", "module.go")); !os.IsNotExist(err) {
		t.Fatalf("dry-run should not create output files, got err=%v", err)
	}
}
