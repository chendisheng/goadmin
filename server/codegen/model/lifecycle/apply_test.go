...
func TestApplyPlanAndResultRoundTrip(t *testing.T) {
	...
	plan := ApplyPlan{
		...
		Force:        false,
		PolicyStore:  PolicyStoreCSV,
		Generation:   []ApplyItem{{Module: "inventory", Kind: "source-file", Path: "server/modules/inventory/module.go", Stage: ApplyStageGenerate, Action: "created", Managed: true}},
	...
	result := ApplyResult{
		...
		Generation: ApplyGenerationReport{
			...
			Status:     "succeeded",
			Created:    []ApplyItem{{Module: "inventory", Kind: "source-file", Path: "server/modules/inventory/module.go", Stage: ApplyStageGenerate, Action: "created", Managed: true}},
		},
