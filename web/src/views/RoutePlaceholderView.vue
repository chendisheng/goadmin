...
const pageTitle = computed(() => (typeof route.meta.title === 'string' && route.meta.title.trim() !== '' ? route.meta.title : '页面占位'));
const componentName = computed(() => String(route.meta.componentName || '未知组件'));
    ...
    <el-card class="page-card" shadow="never">
      <template #header>
        <div class="page-card__header">
          <span>{{ pageTitle }}</span>
          <el-tag effect="plain" round type="warning">Dynamic Route</el-tag>
        </div>
      </template>
      ...
      <div class="route-placeholder-page__body">
        <el-alert
          title="该路由已由后端菜单驱动注册"
          description="当前页面用于承接尚未实现的业务模块，占位逻辑会在后续 Phase 13/14 中替换为真实页面。"
          type="info"
          show-icon
          :closable="false"
        />
        ...
        <el-descriptions :column="1" border size="small" class="route-placeholder-page__meta">
          ...
          <el-descriptions-item label="外链地址">{{ routeLink }}</el-descriptions-item>
        </el-descriptions>
